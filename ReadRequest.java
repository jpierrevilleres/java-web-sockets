 ...
 * output, and then closes the connection. * The program continues to run, and
 * the server continues to listen for new connections, until the program is
...
public class ReadRequest {
	...
	private static void handleConnection(Socket connection) {
		...
		String httpRootDir = "/home/jp/www/";
		client = connection;
		try {
			...
			req = in.readLine();
			line = req;
			// loop through and discard rest of request line = req;
			while (line.length() > 0) {
				line = in.readLine();
			}
			...
			// create File object
			File f = new File(httpRootDir + requestedFile); // check to see if file exists
			if (!f.canRead()) {
		...
		// GET <filename> HTTP/1.x req = in.readLine();
	...
	//The first line is a status message from the server to the client. //The second line holds the connection type.
